...
2. JNLP içinde ana bileşenler `Startupv3.*.jar` vb. JAR’lara işaret ediyor.
3. JAR analizi sırasında `com.lbs.start.SocketToken` sınıfında `new ServerSocket(port)` ve `new ObjectInputStream(...).readObject()` çağrıları belirlendi; gelen mesajlar filtre/allow-list olmadan işleniyor.
...
HEAD attığımızla birlikte python kodunda girdiğimiz link işlenmiş, oraya girdiğimiz şey her ne olursa olsun bir önemi yok, o bize sadece `DOCUMENT_URI` 'ye HEAD işlemlerinin kayıt edildiğini gösteriyor, içeride bir deserialization işlem yapılma ihtimali eğer doğru yapılandırma yoksa apache tomcat ve Java WEB applerde çok yüksek ki sunucu PUT veya DELETE gibi HTTP methodlarına izin verdiği için bu işleme çok müsait bir ortam var. Normalde HEAD methodu ile RCE yapılmaz, uzun uzun araştırdım ancak böyle bir CVE'ye denk gelmedim hiç. 
