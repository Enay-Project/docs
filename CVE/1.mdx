...
```java
 ...
 530:   mtd3.invoke(applet, new Object[] { docURI });
```
...
Uygulamanın **JNLP akışı** sırasında istemciye verdiği `Startup.jar` içindeki `JLbsStartup ve websocket` başlatıcı, JNLP’de ilettiğim `DOCUMENT_URI` parametresini **arkada doğrudan tüketiyor**. Sunucu tarafında bu değer, istenen belge/konfigürasyonu çekmek için zincirin içine giriyor ve \*\*gelen veriyi \*\*`ObjectInputStream.readObject()`**ile deserialize eden bir kod yoluna**  temas ediyor.\
Önceki sürümde **kimlik doğrulaması olmadan**, `/logo/smart/runapp` endpoint’i **istek gövdesini metoddan bağımsız** okuyordu ben de `HEAD`**isteğiyle**  `application/x-java-serialized-object` gövde gönderip **gadget chain**’i çalıştırdım. Yani sorun **JNLP parametresi ile sunucu tarafı deserialize** köprüsünden kaynaklanıyordu, ağ katmanında method/Content-Type fark etmeyen bir okuma vardı.\
JNLP’deki `DOCUMENT_URI`den server-side fetch sırasında `ObjectInputStream.readObject()`’a akan veri, metod-agnostik body okuma nedeniyle uzaktan ve auth’suz tetiklenebiliyordu, **HEAD + serialized body ile RCE elde ettim.**
...
HEAD    | >1000B .ser  | application/x-java-serialized-object | WAF
...
ancak **kök neden** (güvensiz deserialization) kod tarafında kalıcı olarak kaldırılmadıysa risk mimaride hala var kabul edilmeli.
