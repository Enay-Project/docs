...
Whonix üzerindeki Tor, Gateway makinesinin `/usr/local/etc/torrc.d` dizininde saklanan yapılandırma dosyaları ile kontrol edilir. "Anon Connection Wizard"ı (Anonim Bağlantı Sihirbazı) tekrar çalıştırarak yeni torrc dosyalarının otomatik yazılmasını sağlayabiliriz. Bu sihirbazı Sistem (System) menüsü altında bulabilirsiniz.
