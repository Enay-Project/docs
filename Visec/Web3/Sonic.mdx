...
Ufak uyarı: Göstermiş olduğum örnek origin altyapısını kullanmakta.

Öncelikle hedefimiz bu

![Ekrangörüntüsü2025 08 17013651 Pn](/images/Ekrangörüntüsü2025-08-17013651.png)

170k dolar değerinde wrapped coin bulunuyor, sonic altyapısını kullanan bu tokeni bende ilk defa zafiyet ararken buldum.
...
- cast keccak "PROPOSER\_ROLE" # =\> PROPOSER\_ROLE hash
- cast keccak "EXECUTOR\_ROLE" # =\> EXECUTOR\_ROLE hash
- cast keccak "TIMELOCK\_ADMIN\_ROLE" # =\> TIMELOCK\_ADMIN\_ROLE hash
...
1. **Timelock.grantRole(PROPOSER\_ROLE, biz)** → ileriye dönük planlama serbestisi
2. **Timelock.grantRole(EXECUTOR\_ROLE, 0x000…000 veya biz)** → herkes/biz execute edebilelim
...
[https://remix.ethereum.org/](https://remix.ethereum.org/) kendi metamask hesabınla giriş yap, 3 4 dolar birşey attım içine denemek için, burdada aynı adresi yokladığımda bu adrese hiçbir allahın kulu konmamıştı, yani gene gene gene istediğim adrese istediğim şekilde tokeni yollayabiliyodum.
...
Okuduğunuz için teşekkürler. Aşağıya exploit kodlarını bıraktım. Bunları incelersiniz bi.
...
<Tip>
  **Timelock protokol zafiyeti değil, eksik initialization zafiyeti.**
</Tip>